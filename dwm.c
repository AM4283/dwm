...
static void markfocus(const Arg *arg);
...
void keypress(XEvent *e) {
  ...
  for (i = 0; i < LENGTH(keys); i++) {
    if (keysym == keys[i].keysym && keys[i].chain == -1 &&
      ...
      keychain = keysym;
      for (j = 0; j < LENGTH(modifiers); j++)
        XGrabKey(dpy, AnyKey, 0 | modifiers[j], root, True, GrabModeAsync,
                 GrabModeAsync);
    } else if (!current && keysym == keys[i].keysym && keychain != -1 &&
               keys[i].chain == keychain && keys[i].func)
      keys[i].func(&(keys[i].arg));
  }
  if (!current) {
    keychain = -1;
...
void tagnextmon(const Arg *arg) { tagothermon(arg, 1); }

void tagprevmon(const Arg *arg) { tagothermon(arg, -1); }
...
void setmark(Client *c) {
  if (c == mark)
    return;
  if (mark) {
    XSetWindowBorder(
        dpy, mark->win,
        scheme[mark == selmon->sel ? SchemeSel : SchemeNorm][ColBorder].pixel);
    mark = 0;
  }
  if (c) {
    XSetWindowBorder(
        dpy, c->win,
        scheme[c == selmon->sel ? SchemeSel : SchemeNorm][ColMark].pixel);
    mark = c;
  }
}
...
void tagswapmon(const Arg *arg) {
  ...
  for (c = mc; c; c = next) {
    ...
    c->mon = selmon;
    c->tags =
        selmon->tagset[selmon->seltags]; /* assign tags of target monitor */
    attach(c);
    attachstack(c);
    if (c->isfullscreen) {
      resizeclient(c, c->mon->mx, c->mon->my, c->mon->mw, c->mon->mh);
